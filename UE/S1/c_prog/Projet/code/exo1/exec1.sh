...
$(basename "$0") [-h] [-q] [Dossier de tests (01, 02, ..., 10)] -- ce programme compile et exécute le code du fichier exo1.c sur chacun des tests précisés en argument.
...
if [ ! -f $DIR/exo1.c ]; then
    echo "Le fichier $DIR/exo1.c n'existe pas ou n'est pas dans le même dossier que ce script.!"
    exit 2
fi
...
for i in $s 
do
if [ ! -f $DIR/tests/exo1/$i/input ]; then
    echo "Le dossier de test $DIR/tests/exo1/$i précisé en argument n'existe pas ou ne contient pas de fichier nommé input!"
    exit 2
fi
if [ ! -f $DIR/tests/exo1/$i/output ]; then
    echo "Le dossier de test $DIR/tests/exo1/$i précisé en argument n'existe pas ou ne contient pas de fichier nommé output!"
    exit 2
fi
done
# Compile le code
...
do
  ...
  instancefile=$DIR/tests/exo1/$i/input
  expectedfile=$DIR/tests/exo1/$i/output
  ...
  if ! $QUIET
  then
    # Lance le code exo1, s'arrête au bout de 3s max et tout ce qui est envoyé dans stdout est à la fois transmis à la console et dans la variable $solution.
    solution=$(timeout 3s $DIR/exo1 < $instancefile | tee /dev/tty)
  else
    # Mode -q : Lance le code exo1, s'arrête au bout de 3s max et tout ce qui est envoyé dans stdout est transmis à la variable $solution.
    solution=$(timeout 3s $DIR/exo1 < $instancefile)
  fi
