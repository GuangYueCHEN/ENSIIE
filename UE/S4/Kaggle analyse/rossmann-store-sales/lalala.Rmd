...
#测试#
n <- dim(train)[1]
NewTrainSize <- round(n*.9)
NewTrainIndex <- sample(1:n, size = NewTrainSize)
NewTrain <-  train[NewTrainIndex,]
NewTest <-  train[-NewTrainIndex,]
#测试#
...
trainHex<-as.h2o(NewTrain)
## Set up variable to use all features other than those specified here
features<-colnames(NewTrain)[!(colnames(NewTrain) %in% c("Id","Date","Sales","logSales","Customers"))]
...
```{r}
library(MLmetrics)
testHex<-as.h2o(NewTest)
predictions<-as.data.frame(h2o.predict(rfHex,testHex))
```

```{r}
pred <- expm1(predictions[,1])
cat("RMPSE =", RMSPE(pred,NewTest$Sales))
