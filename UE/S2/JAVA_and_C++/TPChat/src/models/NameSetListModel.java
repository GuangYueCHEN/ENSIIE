...
public class NameSetListModel extends AbstractListModel<String>
{
	...
	public boolean add(String value)
	{
		 ...
		 * 	block
		 * 	- if nameSet is actually modified observers should be
		 * 	notified with fireContentsChanged(this, 0, nameSet.size()-1)
		 */
		synchronized(nameSet) { 
		if(value==null||value.length()==0||nameSet.contains(value)) {
		return false;
		}else {
	...
	public boolean remove(int index)
	{
		 ...
		 */
		synchronized(nameSet) {  
		if(nameSet.remove(this.getElementAt(index))) {
			fireContentsChanged(this, 0, nameSet.size()-1);
			return true;
		}
		return false;
	...
	public String getElementAt(int index)
	{
		 ...
		 */
		synchronized(nameSet) { 
			if(index>=nameSet.size())return null;
