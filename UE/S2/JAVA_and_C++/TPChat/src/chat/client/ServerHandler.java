	                     ...
	                     Logger parentLogger)
	{
		...
		if (in != null)
		{
			...
			try{
				...
				logger.info("ObjectInputStream instantiation");
			}catch(IOException e){
				logger.severe("ServerHandler1: " + Failure.CLIENT_INPUT_STREAM);
		...
		if (out != null)
		{
			...
			logger.info("ServerHandler: creating user output ...down ");
		}
		else
		{
	...
	public void run()
	{
		...
		while (commonRun.booleanValue())
		{
			...
			try {
				...
				logger.warning("ServerHandler: InvalidClass");
			}
			catch(StreamCorruptedException e) {
				logger.warning("SyncFailed");
			}
			catch(OptionalDataException e) {
				logger.warning("OptionalData");
			}
			catch (IOException e) {
			...
			if ((message != null))
			{
				...
				switch (userOutType)
				{
					...
					try {
						...
						error=true;
						logger.warning("ServerHandler:Error displaying message to user");
					...
					default:
						// TODO userOutPW...
						 ...
						 if (userOutPW.checkError()) {
								logger.warning("ServerHandler: Error displaying message to user"); // OTHER ???
								error = true;
							}; 
