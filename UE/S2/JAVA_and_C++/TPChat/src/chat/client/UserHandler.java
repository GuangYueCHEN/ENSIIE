	...
	public void run()
	{
		...
		while (commonRun.booleanValue())
		{
			...
			try {
				...
				logger.severe("UserHandler: Impossible de lire user IN");
				break;
			}
			...
			if (userInput != null)
			{
				/*
				 * TODO 
				 * DOWN Sends user input to server using the server print writer
				...
				/*
				 * TODO
				 * DOWN check if user has typed the byeCmd from the Vocabulary
				 * and if so break the loop
				 */
				if(serverOutPW.checkError()){
					logger.severe("ChatClient: " + Failure.OTHER); 
					break;
				}
