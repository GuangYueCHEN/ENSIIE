...
class UserHandler implements Runnable
{
	...
	public void run()
	{
		 ...
		 * 		- also check for special commands such as byeCmd from
		 * 		the Vocabulary
		 */
		while (commonRun.booleanValue())
		{
			...
			try {
				...
				logger.severe("UserHandler: Impossible de lire user IN"); // CHANGER????
				break;
			}  
			...
			if (userInput != null)
			{
				...
				if (serverOutPW.checkError()) {
					logger.severe("ChatClient: " + Failure.OTHER); // OTHER ???
					break;
				}
				/*
