...
void allreduce_blocking(int Nloc, int Ncalc, float *tab_loc, float *tab_res, float *tab_calc) 
{
    ...
    MPI_Allreduce(tab_loc, tab_res, Nloc, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD);
}
...
void allreduce_nonblocking(int Nloc, int Ncalc, float *tab_loc, float *tab_res, float *tab_calc) 
{
    ...
    MPI_Iallreduce(tab_loc, tab_res, Nloc, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD, &req);
    MPI_Wait(&req,&test);
}
...
void allreduce_nonblocking_pipelined2(int Nloc, int Ncalc, float *tab_loc, float *tab_res, float *tab_calc) 
{
    ...
    MPI_Iallreduce(&tab_loc[0], &tab_res[0], nloc1, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD, &(req[0]));

    MPI_Iallreduce(&tab_loc[nloc1], &tab_res[nloc1], Nloc - nloc1, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD, &(req[1]));
...
void allreduce_nonblocking_pipelined4(int Nloc, int Ncalc, float *tab_loc, float *tab_res, float *tab_calc) 
{
    ...
    int nloc1 = Nloc / 4;
    int nloc2 = Nloc / 2;
    int nloc3 = nloc1 + nloc2;

    MPI_Iallreduce(&tab_loc[0], &tab_res[0], nloc1, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD, &(req[0]));

    MPI_Iallreduce(&tab_loc[nloc1], &tab_res[nloc1], nloc2 - nloc1, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD, &(req[1]));

    MPI_Iallreduce(&tab_loc[nloc2], &tab_res[nloc2], nloc3 - nloc2, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD, &(req[2]));

    MPI_Iallreduce(&tab_loc[nloc3], &tab_res[nloc3], Nloc - nloc3, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD, &(req[3]));
    
    MPI_Waitall(4,req,test);
}
