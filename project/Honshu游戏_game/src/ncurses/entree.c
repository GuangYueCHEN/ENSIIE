...
static void action(t_hs * hs) {
	...
	if (FLAG_ISSET(tuile->flags, TUILE_SUR_CARTE)) {
		...
		if (grille_can_insert(grille, tuile, x, y)) {
			...
			hs->entreur->tuile = (hs->entreur->tuile + 1) % hs->nb_tuiles;
		}
	}
}
...
void entrer(t_hs * hs) {
	...
	switch (ch) {
		...
		case 'z':
			...
			break ;
		case 'e':
			hs->entreur->tuile = (hs->entreur->tuile == hs->nb_tuiles - 1) ? 0 : hs->entreur->tuile + 1;
			break ;
		case 'a':
			...
			break ;
		case 'r':
			if (!FLAG_ISSET((hs->tuiles + hs->entreur->tuile)->flags,
					TUILE_SUR_CARTE)) {
				tuile_rotation(hs->tuiles + hs->entreur->tuile);
			}
			break ;
		case ' ':
			action(hs);
